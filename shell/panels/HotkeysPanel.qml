...
// (see services/Hotkeys.qml for how the file is read). Sections flow into three columns of roughly equal height,
// in file order; the layout is computed once from the full list so typing in the search box only hides rows and
// cards — nothing jumps around. Esc / click outside / SUPER+K again closes.
Popup {
    id: root
    name: "hotkeys"
    ...
    onShownChanged: if (shown) { input.text = ""; input.forceActiveFocus() }
    ...
    ColumnLayout {
        ...
        // ── three columns of section cards ──
        RowLayout {
                            ...
                            Column {
                                ...
                                Repeater {
                                    ...
                                    Item {
                                        id: row
                                        required property var modelData
                                        width: parent.width; height: Math.max(27, desc.implicitHeight + 8)
                                        Label {
                                            id: desc
                                            text: row.modelData.desc; font.pixelSize: 12
                                            wrapMode: Text.Wrap; maximumLineCount: 2     // a long description wraps once, then elides
                                            anchors { left: parent.left; right: keys.left; rightMargin: 14; verticalCenter: parent.verticalCenter }
                                        }
                                        Row {
                                            id: keys
        ...
        // ── footer ──
        Label { Layout.fillWidth: true; font.pixelSize: 11; color: Theme.outline
