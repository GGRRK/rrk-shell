...
Popup {
    ...
    Column {
        ...
        ListView {
            ...
            // Mouse wheel / two-finger scroll moves the strip sideways: a horizontal Flickable only reacts to
            // horizontal wheel deltas, so a plain mouse wheel did nothing here. A click-transparent MouseArea on
            // top of the cards (acceptedButtons: NoButton = presses/drags fall through to the cards and the
            // Flickable) is the same pattern as the bar's volume/workspace wheels, which work on every device
            // here — the WheelHandler used before did not fire with a real mouse. One notch = half a card.
            MouseArea {
                anchors.fill: parent; z: 2
                acceptedButtons: Qt.NoButton
                onWheel: w => {
                    let d = w.angleDelta.y !== 0 ? w.angleDelta.y : w.angleDelta.x          // wheel / two-finger vertical, else horizontal
                    if (d === 0) d = (w.pixelDelta.y || w.pixelDelta.x) * 8                   // pixel-only touchpads
                    ...
                    strip.contentX = Math.max(min, Math.min(max, strip.contentX - d * 1.125))
                    w.accepted = true
                }
            }
            delegate: Item {
